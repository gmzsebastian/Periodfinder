...
h_guess = 21.0     # Line Height
c_guess = 6563.0   # Line Center (H-alpha)
w_guess = 15.38    # Line Width
...
Guess1 = np.array([h_true, c_true, w_true, f_true])

# Log prior funtion, define the priors here
def lnprior(theta):
    h, c, w, lnf = theta
    if 5 < h < 30 and 6550.0 < c < 6575.0 and 5.0 < w < 20.0 and -5 < lnf < 1:
        return 0.0
    return -np.inf
...
ndim, nwalkers, stepsize = 4, 500, 1E-3
...
print "Plotting"
fig = corner.corner(samples, labels=["$h$", "$c$", "$w$", "$f$"], truths=[h_true, c_true, w_true, f_true])
fig.savefig("Triangle_All.png")
plt.clf()

# Obtain the parametrs of the best fit
h_mcmc, c_mcmc, w_mcmc, f_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),
...
B = str(np.around(c_mcmc[0], decimals=3))
...
Berr = str(np.around(c_mcmc[1], decimals=3))
...
plt.plot(x, gaussian(x, *Optimized), lw=1, c='r', ls='--', label='Optimized')
plt.plot(x, gaussian(x, h_mcmc[0], c_mcmc[0], w_mcmc[0]), c = 'b', label = 'MCMC Fit')
plt.plot(x, gaussian(x, h_guess, c_guess, w_guess), c = 'k', label = 'Guess')
ToPlot("Fit_All.jpg")
